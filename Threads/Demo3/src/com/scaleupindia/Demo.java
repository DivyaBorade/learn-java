...
public class Demo {
	public static void main(String[] args) {
		...
		EmployeeRepository[] employeeRepositories = { new EmployeeRepositoryImpl1() , new EmployeeRepositoryImpl2() , new EmployeeRepositoryImpl3() };
		...
		for (int i = 0; i < employeeRepositories.length; i++) {
			...
			System.out.println( Thread.currentThread().getName() + " -> Just after using new keyword -> Name: " + employeeServices[i].getName() + ", isAlive: " + employeeServices[i].isAlive() + ", state: " + employeeServices[i].getState());
			employeeServices[i].start();
			System.out.println( Thread.currentThread().getName() + " -> Just after using start method -> Name: " + employeeServices[i].getName() + ", isAlive: " + employeeServices[i].isAlive() + ", state: " + employeeServices[i].getState());
		}
		
		for(int i = 0; i < employeeServices.length; i++) {
			try {
				employeeServices[i].join();
				System.out.println( Thread.currentThread().getName() + " -> Just after using join method -> Name: " + employeeServices[i].getName() + ", isAlive: " + employeeServices[i].isAlive() + ", state: " + employeeServices[i].getState());
